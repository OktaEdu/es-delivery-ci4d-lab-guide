...
The Partner OU contains users that are not synced to Okta via Active Directory. You will use these users to test the AD FS integration.
...
In this section, you confirm that your AD FS can authenticate partners from Active Directory.

1. Open Chrome and navigate to to https://adfs.oktaice.local/adfs/ls/idpinitiatedsignin

2. Login as Diane Smith (login: OKTAICE\dsmith), who is a user in the Partner OU.

3. Verify that sign in is successful.

4. Sign out and close your browser.

### Download the AD FS Signing Certificate

The signing certificate is used by AD FS to sign its SAML assertions. In this section, you download the signing certificate. In a future step, we will upload this certificate to Okta. Okta will use the signing certificate during the app initiated sign-on to confirm that the SAML assertion is generated by AD FS.

1. From the Windows toolbar in the VM, open `AD FS Management`
2. On the left menu, go to `AD FS` > `Service` > `Certificates`
3. On the `Certificates` panel, right-click the certificate under `Token-signing` and click `View Certificate`.
4. Click the `Details` tab.
5. Click `Copy to File`
6. The Windows Certificate Export Wizard is launched. Follow the Wizard instructions to export the certificate using the default settings. Save the file as `adfs.cer` under the `Desktop` folder.
7. Close the certificate pop-up and minimize the AD FS Management utility since we will use the utility later to integrate AD FS and Okta.
8. Navigate to your Desktop and confirm that the `adfs.cer` file was saved there.
