...
![AD FS Management](./.tour-resources/adfsmanagementicon.png)
...
4. Next to **User matches** select `Domain list on login`
5. In the field that appears beneath, type in `oktaice.local` and click `add oktaice.local...`
6. In the **Use this identity provider** section, keep `Use specific IdP(s)` selected.
7. In the **IdPs** section at the bottom, remove `Okta` by clicking the `X` and then type and select `AD FS`
8. Click `Create rule`
9. Finally, click `Activate` to activate the rule.
...
2. In the **Select Data Source** step, select `Import data about the relying party from a file` and click `Browse`
3. In the file browser window that opens, navigate to the `Downloads` folder and double-click the `metadata.xml` file you downloaded from Okta.
4. Click `Next`
5. Enter `Okta Ice` as the **Display Name** and click `Next`
6. Continue clicking `Next`, taking the default values, until the Wizard reaches the last step.
7. Click `Close`
...
1. Return to `AD FS` > `Relying Party Trusts` in the **AD FS Management** window.
2. Right-click `Okta Ice` and then click `Properties`
3. Click the `Identifiers` tab.
4. Enter `OktaIce` as `Relying party identifier` and click `Add` to add it to the `Relying party identifier` text box.
...
2. Enter `diane.smith@oktaice.local` for `Username`
...
7. You'll now be redirected to AD FS for authentication.

8. Enter Enter `diane.smith@oktaice.local` as the **username** `Tra!nme4321` as the **password**
